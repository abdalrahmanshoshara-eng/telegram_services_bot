...
def test_services_are_grouped_into_categories():
    ...
    assert len(bot.services_for_category("documents")) == 5
    assert len(bot.services_for_category("images")) == 3
    assert len(bot.services_for_category("data")) == 2
...
def test_number_selection_is_local_to_the_chosen_category():
    ...
    assert bot.service_by_category_number("data", 2)["id"] == "colornote_to_html"
    assert bot.service_by_category_number("data", 3) is None
